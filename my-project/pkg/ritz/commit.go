...
func createCommit(commitContent, commitMessage string) error {
	...
	if err := ioutil.WriteFile(".ritz/refs/heads/master", []byte(headContent), 0644); err != nil {
		return err
	}
...
func VerifyCommit() error {
	...
	headParts := strings.Split(string(headContent), "\n")
	if len(headParts) < 3 {
		return fmt.Errorf("invalid HEAD reference")
	}
	commitHash := strings.TrimSpace(headParts[1])
